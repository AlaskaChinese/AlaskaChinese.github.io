# LESSWARE Website

Bilingual static website for **LESSWARE**, an independent design & engineering studio exploring focused electronic objects.

## Live site
- Chinese: https://alaskachinese.github.io/
- English: https://alaskachinese.github.io/en.html

## Products
- **L–01 / LISTEN** — E-Ink reading + Hi-Fi player
- **D–01 / DRAFT** — foldable E-Ink writing machine

## Contact
- WeChat: `AlaskaAzusagawa`
- Email: `alaskachinese@outlook.com`
- GitHub: `AlaskaChinese`
- Address: 重庆市南岸区学府大道

## Deployment
This repository is the GitHub Pages user site `AlaskaChinese/AlaskaChinese.github.io`.
The static site is served from the `main` branch root.
